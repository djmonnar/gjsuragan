...
function buildRogenEditText(){
  const lines = [g('en'), g('ep'), g('ea'), g('ed'), g('er')].filter(Boolean);
  return lines.join('\n');
}

function fillRogenEditText(){
  const box = document.getElementById('e-rozen');
  if(!box) return;
  box.value = buildRogenEditText();
}

function copyRogenEditText(){
  const box = document.getElementById('e-rozen');
  if(!box) return;
  if(!box.value.trim()) fillRogenEditText();
  navigator.clipboard.writeText(box.value || '').then(()=>toast('로젠택배용 텍스트 복사 완료','ok'));
}

function applyRogenEditText(){
  const box = document.getElementById('e-rozen');
  if(!box){ return; }
  const raw = (box.value || '').trim();
  if(!raw){ toast('붙여넣을 내용이 없습니다','er'); return; }

  const lines = raw.split(/\n+/).map(v=>v.trim()).filter(Boolean);
  const flat = raw.split(/\t+/).map(v=>v.trim()).filter(Boolean);
  const vals = flat.length >= 3 ? flat : lines;

  if(vals[0]) g2('en', vals[0]);
  if(vals[1]) g2('ep', vals[1].replace(/\s+/g,''));
  if(vals[2]) g2('ea', vals[2]);
  if(vals[3]) g2('ed', vals[3]);
  if(vals[4]) g2('er', vals[4]);

  fillRogenEditText();
  toast('수정 탭에 붙여넣기 반영 완료','ok');
}
