(function () {
  ...
  let device = '', data = null, busy = false, failed = false, getNow = Date.now, shownDay = '', refreshAgain = false;
  function render() {
    const now = getNow();
    const current = data?.date === U.date(now) ? data : null;
    const ready = current?.state === 'ready';
    const rows = current?.bookings || [];
    ...
    $('bookings-add').disabled = !device || !current || current.state === 'unconfigured';
    $('bookings-store').textContent = `${current?.storeName || '돌담명가'} · 예약${current?.demo ? ' · 미리보기' : ''}`;
    $('bookings-summary').innerHTML = ready || rows.length
      ? `<strong>${active.length}<small>건</small></strong><span class="att-bookings-divider"></span><strong>${active.reduce((n, row) => n + row.adults + row.children, 0)}<small>명</small></strong><span class="att-meta">${ready ? '오늘 방문 기준' : '직접 등록 기준'}</span>`
    ...
    const rowHtml = row => `<article class="att-booking-row ${row === next ? 'next' : ''} ${!row.active ? 'cancelled' : ''}"><div class="att-booking-time">${U.esc(row.time || '시간 미정')}${row === next ? '<small>다음 예약</small>' : ''}</div><div class="att-booking-info"><div class="att-booking-name"><strong>${U.esc(row.name || '이름 확인 필요')}</strong><span>${row.adults + row.children}명${row.children ? ` <small>(어린이 ${row.children})</small>` : ''}</span></div><p>${U.esc(row.menuItems.length ? row.menuItems.map(item => `${item.name} ${item.count}`).join(' · ') : row.itemName || '메뉴 정보 없음')}</p>${row.seat ? `<p class="att-booking-seat">좌석 · ${U.esc(row.seat)}</p>` : ''}<span class="att-booking-state">${row.origin === 'manual' ? '직접 등록' : '네이버'} · ${labels[row.status] || '상태 확인'}</span></div></article>`;
    $('bookings-list').innerHTML = active.map(rowHtml).join('')
      + (ready && !active.length ? '<div class="att-bookings-empty">오늘 방문 예정인 예약이 없습니다.</div>' : '')
  ...
  async function sync() {
    ...
    } finally { busy = false; render(); if (refreshAgain) { refreshAgain = false; sync(); } }
  }
  window.AttendanceBookings = {
    ...
    clear() { device = ''; data = null; failed = false; window.AttendanceBookingForm?.clear(); render(); }
  };
  $('bookings-add').onclick = () => window.AttendanceBookingForm?.open({
    date: U.date(getNow()), menuOptions: data?.menuOptions, scope: 'tablet',
    save: input => U.request('kiosk.booking.create', input, { device }),
    onSaved: async result => {
      $('bookings-saved').hidden = false;
      $('bookings-saved').textContent = `${result.useDate === U.date(getNow()) ? '오늘' : result.useDate} 예약을 등록했습니다.${result.useDate !== U.date(getNow()) ? ' 해당 날짜의 예약은 admin에서 확인할 수 있어요.' : ''}`;
      await sync();
    }
  });
