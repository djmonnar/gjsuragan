  ...
  function bindPayType(dialog) {
    ...
    const sync = () => {
      ...
      if (salary) {
        ...
        dialog.querySelectorAll('.att-salaried-only').forEach(box => { box.hidden = isHourly; });
        dialog.querySelectorAll('.att-salaried-only input').forEach(input => { input.disabled = isHourly; });
      }
    };
