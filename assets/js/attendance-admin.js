(function () {
  ...
  const dailySettingText = e => {
    ...
    // 초과 시급을 넣어두면 첫 단위만 정액이고 그 뒤부터 시급으로 센다.
    const extra = !e.overtimePay ? ''
      : e.overtimeHourlyRate
        ? ` · 기준 뒤 첫 ${unit}분 ${U.money(e.overtimePay)} · 이후 시급 ${U.money(e.overtimeHourlyRate)}`
        : ` · 기준 뒤 ${unit}분마다 ${U.money(e.overtimePay)}`;
  ...
  function dailyPayrollHtml(rows) {
    ...
    return `<section class="att-payroll att-daily-payroll"><div class="att-section-heading"><div><h3>일일근무자 <span class="att-meta">${rows.length}건</span></h3><p class="att-subtitle">한 건이 한 사람입니다. 이름을 적고 금액을 확인한 뒤 지급하세요.</p></div></div>
      ...
      <p class="att-note">일일근무자는 시간이 아니라 하루 단위로 계산합니다. <b>기준 근무시간</b>을 넘기면 <b>추가 급여 단위</b>마다 금액이 더 붙습니다 (기본 8시간 · 30분 · 30분을 넘겨야 첫 회가 붙습니다). <b>초과 시급</b>을 넣으면 첫 단위만 정액이고 그 뒤부터 시급으로 계산합니다. <b>예정 출근 시각</b>을 정해 두면 그보다 일찍 찍은 시간은 초과로 세지 않습니다. 특수일 배율은 붙지 않으니, 더 드릴 금액은 그 기록의 <b>이 근무의 일당</b>을 직접 고쳐 주세요.<br>3.3%를 체크한 기록은 뗀 금액이 표시되고 <b>금액 복사</b>도 그 금액으로 복사됩니다.</p>`;
  }
