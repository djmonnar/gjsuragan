(function () {
  ...
  const dailySettingText = e => {
    ...
    const extra = e.overtimePay
      ? ` · 기준 뒤 ${e.overtimeUnitMinutes || DEFAULT_OVERTIME_UNIT_MINUTES}분마다 ${U.money(e.overtimePay)}` : '';
    ...
    return `${base}시간 기준 · ${e.halfDayPay ? `${minutesToClock(e.halfDayBeforeMinutes || DEFAULT_HALF_DAY_BEFORE_MINUTES)} 전 퇴근은 반타임 ${U.money(e.halfDayPay)}` : '반타임 없음'}${extra}`;
  };
