(function () {
  ...
  const dailySettingText = e => {
    ...
    if (e.dailyMode === 'prorate') {
      ...
      return `${base}시간 기준 · 일한 시간 비례 (${grace}분까지는 전액)${extra}`;
    }
    return `${base}시간 기준 · ${e.halfDayPay ? `${minutesToClock(e.halfDayBeforeMinutes || DEFAULT_HALF_DAY_BEFORE_MINUTES)} 전 퇴근·이후 출근은 반타임 ${U.money(e.halfDayPay)}` : '반타임 없음'}${extra}`;
  };
  ...
  // 예정 출근 시각은 여러 개다. 옛 직원·기록은 시각 하나만 들고 있어 그 값도 같이 읽는다.
  const scheduledList = value => Array.isArray(value?.scheduledStarts) ? value.scheduledStarts
    : (Number(value?.scheduledStartMinutes) > 0 ? [Number(value.scheduledStartMinutes)] : []);
  const scheduledText = value => scheduledList(value).map(minutesToClock).join(' · ');
  ...
  function renderEmployees() {
    ...
    $('att-content').innerHTML = `<div class="att-section-heading"><div><h3>직원 목록 <span class="att-meta">${list.length}명</span></h3><p class="att-subtitle">직원 정보와 급여 계좌를 관리하세요.</p></div></div><div class="att-employee-cards">${list.length ? items.map(e => e.__header ? `<h4 class="att-part-title att-part-row">${e.__header}<span>${e.count}명</span></h4>` : `<article class="att-employee-card">
      ...
      <div class="att-card-foot"><span class="att-meta">주민등록번호 ${e.privateSummary?.residentRegistered ? '등록됨' : '미등록'} · 휴게 ${e.breakMinutes}분${scheduledList(e).length ? ` · 예정 출근 ${scheduledText(e)}` : ''}${e.withholding ? ' · <b>3.3% 원천징수</b>' : ''}${isDailyPaid(e.payType) ? ` · ${dailySettingText(e)}` : ''}</span><div><button class="att-link-button" data-action="edit-employee" data-id="${U.esc(e.id)}">수정</button><button class="att-link-button att-delete-link" data-action="delete-employee" data-id="${U.esc(e.id)}">삭제</button></div></div>
