...
이 챗봇은 일반 고객용이 아니라 내부 관리자용입니다. 카카오톡 말풍선에 고객명, 전화번호, 주소, 요청사항이 출력되므로 직원전용 채널과 내부 단말에서만 사용해야 합니다.
...
## 2. Apps Script 직접 연결 시 Script Properties 등록값

Apps Script 웹 앱 URL을 카카오 스킬에 직접 연결하는 백업 방식에서는 `프로젝트 설정 > 스크립트 속성`에 아래 값을 등록합니다. 현재 권장 방식인 Firebase Functions URL을 쓰는 경우 카카오 조회에는 이 PIN 설정이 필요하지 않습니다. 값은 코드나 GitHub 문서에 직접 적지 말고 Apps Script 속성에만 저장하세요.
...
현재 Firebase Functions 스킬 URL은 빠른 내부 운영을 위해 PIN 인증 없이 응답하도록 설정되어 있습니다. 향후 다시 잠글 때 사용할 수 있는 환경변수는 아래와 같습니다.

| 환경변수 | 용도 | 필수 |
|---|---|---|
| `KAKAO_ADMIN_PIN` | 카카오 챗봇 관리자 인증 PIN. 현재 Functions 기본 흐름에서는 사용하지 않음 | 선택 |
| `KAKAO_ALLOWED_USERS` | 허용할 카카오 user id 목록. 콤마로 구분 | 선택 |
...
직원전용 채널로 운영할 때는 채널명과 설명에 내부용임을 명확히 적어두세요.

- 채널명 예시: `궁중수라간 직원전용`
- 설명 예시: `궁중수라간 내부 업무 확인용 채널입니다.`
- 고객용 채널이나 고객 안내 메시지와 섞지 않습니다.
...
| 블록명 | 발화 예시 | 권장 파라미터 |
|---|---|---|
| 오늘할일 | `오늘할일`, `오늘 할 일`, `할일`, `업무체크` | `mode=tasks` |
...
- 현재 Functions 연결은 PIN 없이 조회됩니다. 카카오 채널을 직원전용으로만 운영하고 스킬 URL을 외부에 공유하지 마세요.
- 운영 보안을 강화하려면 `KAKAO_ALLOWED_USERS` 또는 PIN 인증을 다시 켜서 허용된 카카오 user id만 조회하게 하세요.
- `KAKAO_ADMIN_PIN`을 다시 사용할 경우 외부에 공유하지 말고 주기적으로 바꾸는 것을 권장합니다.
...
2. 카카오 스킬 테스트에서 `오늘배송`을 호출합니다.
   - 기대 결과: `version: "2.0"` 응답과 배송 조회 내용
3. 아래 명령을 각각 테스트합니다.
   - `오늘할일`
   - `오늘배송`
   - `내일배송`
   ...
   - `2026-06-12 배송`
   - `6월 12일 배송`
4. 응답 JSON에 항상 아래 형태가 포함되는지 확인합니다.
...
- Script Properties 누락 여부 확인
  ...
  - `KAKAO_ADMIN_PIN`
- PIN 인증을 다시 켰다면 Functions 환경변수 `KAKAO_ADMIN_PIN` 누락 여부 확인
