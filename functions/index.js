...
exports.api = onRequest({
  ...
  try {
    ...
    if (pathname === '/api/admin/password-reset-link' || pathname === '/admin/password-reset-link') {
      const result = await handlePasswordResetLink(req.body || {}, user);
      sendJson(res, 200, { ok: true, ...result });
      return;
    }
...
const CUSTOMER_APP_URL = 'https://djmonnar.github.io/gjsuragan/customer.html';

// 관리자가 고객의 비밀번호 재설정 링크를 직접 만들어 카톡 등으로 전달할 때 쓴다.
// Firebase 기본 발신 메일이 네이버·다음에서 조용히 버려지는 경우를 우회한다.
// 링크 자체는 Firebase 가 만드는 정식 재설정 링크라, 메일로 받는 것과 똑같이 동작한다.
async function handlePasswordResetLink(body, user) {
  const email = String(body.email || '').trim().toLowerCase();
  if (!email || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) {
    const error = new Error('이메일 주소를 확인해주세요.');
    error.status = 400;
    throw error;
  }
  ...
  try {
    ...
    if (lookupError.code === 'auth/user-not-found') {
      const error = new Error('이 이메일로 가입된 계정이 없습니다. 고객이 다른 주소로 가입했을 수 있습니다.');
      error.status = 404;
      throw error;
    }
    throw lookupError;
  ...
  if (account.disabled) {
    const error = new Error('비활성화된 계정입니다. 계정을 먼저 활성화해주세요.');
    error.status = 403;
    throw error;
  }

  const continueUrl = String(process.env.CUSTOMER_APP_URL || CUSTOMER_APP_URL).trim();
  let link;
  try {
    link = await admin.auth().generatePasswordResetLink(email, { url: continueUrl, handleCodeInApp: false });
  } catch (linkError) {
    // 승인된 도메인이 아니면 돌아갈 주소 없이 기본 링크를 만든다. 재설정 자체는 똑같이 된다.
    if (linkError.code === 'auth/unauthorized-continue-uri' || linkError.code === 'auth/invalid-continue-uri') {
  ...
  logger.info('Password reset link generated by admin', { admin: user.email || '', target: email });
  return {
    email: account.email || email,
    uid: account.uid,
    link,
    hasPassword: (account.providerData || []).some(provider => provider.providerId === 'password'),
