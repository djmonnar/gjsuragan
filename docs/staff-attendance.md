...
1. `admin.html`의 **직원·근태 → 직원 등록**에서 이름, 담당 업무, **근무 층(1층/2층)**, 급여 유형을 선택한다. 시급 아르바이트는 **시급(원)**, 월급 직원은 **월급(원)**을 입력한다. **전체 층 / 1층 / 2층** 탭으로 직원·캘린더·급여를 나눠 확인한다.
2. 매장 태블릿에서 `attendance.html`을 열고 **이 태블릿의 근무 층**을 선택한 뒤 관리자 비밀번호로 한 번 연결한다. 연결 후 관리자 인증은 메모리에서 종료되고 태블릿 전용 토큰만 브라우저에 남는다. `?floor=1`과 `?floor=2` 링크는 최초 연결 층을 미리 선택하며, 연결된 기기는 서버에 지정된 층을 따른다.
...
7. **태블릿 관리 → 층 변경**에서 기존 기기의 층을 바꾸거나 연결을 해제할 수 있다. 태블릿은 지정 층 직원만 조회·출퇴근할 수 있다. 해제된 기기는 다시 관리자가 연결해야 사용할 수 있다.
8. 이전에 등록한 직원·태블릿·근무 기록은 기본 1층이다. 직원의 근무 층은 퇴근 후 변경할 수 있고, 지난 기록은 당시 층을 유지한다. 새 근무는 현재 소속 층으로 기록하며, 관리자 수정도 지난 근무의 층을 바꾸지 않는다. 층별 CSV에도 조회 층을 표시한다.
...
- 직원 목록과 급여 정산은 휴대폰에서 한 열 카드로 표시한다. `계좌 복사`는 하이픈 없는 계좌번호, `금액 복사`는 쉼표·원 표시 없는 정수 금액을 복사한다. 시급은 조회 월·층의 퇴근 완료 기본급, 월급은 현재 약정 월급이다. 유형이 섞이거나 과거 근무 유형과 현재 유형이 다르면 잘못된 금액 복사를 막고 확인을 요청한다. 실제 이체·입금 완료 처리는 하지 않는다.
